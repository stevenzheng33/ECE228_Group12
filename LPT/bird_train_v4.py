...
def main():
  ...
  model = Network(args.init_channels, BIRD_CLASSES, args.layers, criterion)
  ...
  teacher_h = nn.Linear(512 * teacher_w.block.expansion, BIRD_CLASSES).cuda()
  ...
  train_queue_dataset = bird_loader.getData(mode="train")
  valid_queue_dataset = bird_loader.getData(mode="valid")
  external_queue_dataset = bird_loader.getData(mode="test")

  train_queue = DataLoader(dataset=train_queue_dataset, batch_size=args.batch_size, sampler=RandomSampler(train_queue_dataset), num_workers=4,pin_memory=False)
  valid_queue = DataLoader(dataset=valid_queue_dataset, batch_size=args.batch_size, sampler=RandomSampler(valid_queue_dataset), num_workers=4,pin_memory=False)
  external_queue = DataLoader(dataset=external_queue_dataset, batch_size=args.batch_size, sampler=RandomSampler(external_queue_dataset), num_workers=4,pin_memory=False)
